...
display('Pressure = 100000 Pa, Temperature = 650 K')
...
phi_vec = [0.2630, 0.2657, 0.2923, 0.2925, 0.3269, 0.3270, 0.3271, 0.3274, 0.4207, 0.5010, 0.5020, 1.0250, 1.0230, 2.3970];

   
   
for i = 1:npoints
   phi(i) = phi_vec(i);%0.26 +2.2/npoints*(i-1)/0.9;
   ...
   tamul = 1.002;
    ...
    [gas] = PostShock_fr(cj_speed(i)*tamul,P1, T1,x, mech);
    Ta= temperature(gas);
    [ZNDout1] = znd(gas,gas1,fig_num,cj_speed(i)*tamul,fname);
    ind_len_a = ZNDout1.ind_len_ZND;
    [gas] = PostShock_fr(cj_speed(i)*tbmul,P1, T1,x, mech);
    Tb = temperature(gas);
    [ZNDout2] = znd(gas,gas1,fig_num,cj_speed(i)*tbmul,fname);
...
for i = 1:npoints
z = [phi(i); T2(i); P2(i); rho2(i); cj_speed(i); Ts(i); Ps(i); ind_time_CV(i); exo_time_CV(i); theta_effective_CV(i); theta_effective_ZND(i); ind_time_ZND(i); ind_len_ZND(i); exo_time_ZND(i); exo_len_ZND(i); Tf_ZND(i)];
